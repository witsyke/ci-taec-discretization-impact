...
dp.plot <-
  ggplot(data.frame(dp.sample$so2), aes(dp.sample$so2)) + geom_density() + xlab(expression("Concentration of SO"[2])) + ylim(0, 85) + t
dp.plot.crop <-
  ggplot(data.frame(dp.sample$so2), aes(dp.sample$so2)) + geom_density() + xlab(expression("Concentration of SO"[2])) + xlim(13.76, 13.86) + ylim(0, 85) + t
...
# Disrectize and generate histograms of SO2 cocentration
...
histogram.so2.quantile <- disc.sample.quantile %>%
  count(so2) %>%
  ggplot(aes(x = so2, y = n, fill = so2)) + geom_col() + ylab("count") +  xlab(expression("Concentration of SO"[2])) + ylim(0, 10000)  + scale_x_discrete(
...
histogram.so2.kMedian <- (
  disc.sample.kM %>%
    ggplot(aes(x = so2, fill = so2)) + geom_bar() + ylab("count") +  xlab(expression("Concentration of SO"[2])) + scale_x_discrete(
      labels = function(labels) {
        sapply(seq_along(labels), function(i)
          paste0(ifelse(i %% 2 == 1, '', '\n'), sprintf("%.2f", as.numeric(labels[i]))))
      }
    )  + ylim(0, 10000) + t + scale_fill_manual(values = grey_pal()(3))
)
...
# Calculate quality measures for learned Bayesian networks
...
# Actually calculate quality measures for all learned graphs
...
quality.scores$hamming <-
  apply(filenames, 1, calculate.hd, gt = ground.truth)
quality.scores$shd <-
...
causal.effect.probabilites.t2m.wd <- function(data) {
  ...
  # calculate probabilities for realisations of Latitude
  data <- data %>%
    add_count(Latitude) %>%
    mutate(prob_lat = n / n()) %>%
    select(-n)
  
  # calculate probabilities for realisations of DateTime
  data <- data %>%
    add_count(DateTime) %>%
...
causal.effect.prob.t2m.wd <-
  causal.effect.probabilites.t2m.wd(sample.5m.4.cluster)
...
plot.t2m.do.wd65 <- (
  t2m.do.wd65 %>%
    ggplot(aes(
      x = t2m, y = p, fill = t2m
    )) + geom_bar(stat = 'identity') + ylab("P") +  xlab("T2M") + ylim(0, 0.35) + t + scale_fill_manual(values = grey_pal()(4))
)
plot.t2m.do.wd167 <- (
  t2m.do.wd167 %>%
    ggplot(aes(
...
causal.effect.probabilites.pm10.pm2.5 <- function(data) {
  ...
  # calculate conditional probability for so2 by Zone, Type, DateTime, pm2.5
  data <- data %>%
...
plot.pm10.do.pm25.1 <-
  ggplot(do.pm25.1, aes(x = pm10, y = Ppm10, fill = pm10)) + geom_bar(stat =
                                                                        'identity',
                                                                      position = position_dodge(),
                                                                      show.legend = F)  + xlab(expression("PM"[10])) + ylab("P") + ylim(0, 0.6) + t + scale_fill_manual(values = grey_pal()(2))
plot.pm10.do.pm25.2 <-
  ggplot(do.pm25.2, aes(x = pm10, y = Ppm10, fill = pm10)) + geom_bar(stat =
                                                                        'identity', position = position_dodge()) + xlab(expression("PM"[10])) + ylab("P") + ylim(0, 0.6) + t + scale_fill_manual(values = grey_pal()(2))
ggsave(
  plot.pm10.do.pm25.1,
...
# Generate newtworks to illustrate section of CPDAG2 before and after do-operator
...
causal.effect.co.so2 <-
  causal.effect.probabilites.so2.co(sample.10m.3.bin)
